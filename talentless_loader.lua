...
function finishedSong()
    ...
    -- Check if queue is playing and advance to next song
    if _G.isQueuePlaying and _G.playNextInQueue then
        playSound("6493287948", 0.1)
        NotificationLibrary:SendNotification("Success", "Song finished. Playing next in queue...", 2)
        lilgui:Destroy()
        lilgui = nil
        
        wait(1) -- Brief pause between songs
        _G.playNextInQueue()
    else
        -- No queue playing, use original behavior
        playSound("6493287948", 0.1)
